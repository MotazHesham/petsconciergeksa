...
@section('content')
    ...
    <div class="card" @if(app()->getLocale() == 'ar') style="    text-align: right; direction: rtl;" @endif>
        ...
        <div class="card-body" id="cv">
            ...
            <br>
            <center><h4> {{__('cruds.invoice.fields.simple_invoice', [], 'ar')}}</h4></center>
            <center><h4> {{__('cruds.invoice.fields.simple_invoice', [], 'en')}}</h4></center>

            <br>
            <br>
            <center><h4> {{__('cruds.clients.fields.sale', [], 'ar')}}</h4></center>
            <center><h4> {{__('cruds.clients.fields.sale', [], 'en')}}</h4></center>
            ...
            <div  style="text-align: left; direction: ltr;">
                <div  style="    text-align: left; direction: ltr; float: left;">
                    ...
                    <div>
                        <label class="required" for="title">{{__('cruds.clients.fields.date', [], 'en')}}
...
"  for="title">{{__('cruds.clients.fields.address', [], 'ar')}}
            ...
            <center><h4 style="    direction: rtl; margin-top: 105px;"> رقم الفاتورة: {{$invoice->num}} </h4></center>
            <div  style="text-align: left; direction: ltr;
" >
                ...
                <table class=" table table-bordered table-striped table-hover datatable datatable-Permission"
                    ...
                    @foreach($tasks as  $key=>$task)
                        <tr data-entry-id="{{ $task->id }}">
                            ...
                            <td>
                                {{ number_format($task->daily) ?? '' }}

                            </td>
                            <td>
                                {{ number_format(round($task->over_time_hours,2))  ?? '' }}
                            </td>
                            <td>
                                {{ number_format(round($task->over_time_rate,2))  ?? '' }}
                            </td>
                            <td>
                                {{number_format(round($task->over_time_hours * $task->over_time_rate,2))   ?? '' }}
                            ...
                            <td>
                                {{ number_format($task->absense_rate)  ?? '' }}
                            ...
                            <td>
                                ...
                                ?>
                                {{  number_format(round($task->absense_days * $task->absense_rate,2))   ?? '' }}

                            </td>
                            <td>
                                {{ number_format( round(($task->daily * $task->working) +($task->over_time_hours * $task->over_time_rate) -$total_absense),2)  ?? '' }}
                                <?php
                                $x += ($task->daily * $task->working) + ($task->over_time_hours * $task->over_time_rate) - $total_absense;
                                ?>
                            </td>
                            <td>
                                {{ number_format(round((($task->daily * $task->working) + ($task->over_time_hours * $task->over_time_rate) - $total_absense) * 15/100),2)   ?? '' }}
                            </td>
                            <td>

                                {{ number_format(round((($task->daily * $task->working) + ($task->over_time_hours * $task->over_time_rate)- $total_absense) * 15/100 + ( ($task->daily * $task->working) + ($task->over_time_hours * $task->over_time_rate) - $total_absense ),2))   ?? '' }}
            ...
            <div  style="    text-align: left; direction: ltr;" >
                ...
                <table class="table table-bordered"  @if(app()->getLocale() == 'ar') style="width: 50%;direction: rtl; text-align: right;"@else style="width: 50%;"  @endif>
                    ...
                    <tr>
                        ...
                        <th>
                            {{number_format(round($x,2))}} {{ trans('cruds.clients.fields.currency') }}
                        </th>
                    ...
                    <tr>
                        ...
                        <th>
                            {{number_format(round(($x*15)/100),2)}} {{ trans('cruds.clients.fields.currency') }}
                        </th>
                    ...
                    <tr>
                        ...
                        <th>
                            {{number_format(round($x+ (($x*15)/100),2))}} {{ trans('cruds.clients.fields.currency') }}
                        </th>
